"use strict";function share(n){if(!("share"in navigator)||window.isSecureContext===!1){showError("Web Share API not supported.");return}navigator.share({url:n}).then(()=>console.log("Successful share")).catch(n=>showError(n.message))}function jsSaveAsFile(n,t,i){const f=new File([i],n,{type:t}),u=URL.createObjectURL(f),r=document.createElement("a");document.body.appendChild(r);r.href=u;r.download=n;r.target="_self";r.click();URL.revokeObjectURL(u)}function GetLocalStorage(n){return window.localStorage.getItem(n)}function SetLocalStorage(n,t){return typeof n!="string"||typeof t!="string"?(showError("Key/value must be strings"),null):window.localStorage.setItem(n,t)}function TryDetectPlatform(){if(!GetLocalStorage("platform")){const n=document.referrer=="app-info://platform/microsoft-store",t=/(android)/i.test(navigator.userAgent),i=/iphone|ipad|ipod/i.test(navigator.userAgent),r=/huawei|honor/i.test(navigator.userAgent);n?SetLocalStorage("platform","windows"):t?SetLocalStorage("platform","play"):i?SetLocalStorage("platform","ios"):r?SetLocalStorage("platform","huawei"):SetLocalStorage("platform","webapp")}}async function getUserInfo(){try{if(window.location.host.includes("localhost")){const n=await fetch("/dev-env/me.json");if(!n.ok)throw new Error(`HTTP ${n.status}`);const t=await n.json();return t?.clientPrincipal}else{const n=await fetch("/.auth/me");if(!n.ok)throw new Error(`HTTP ${n.status}`);const t=await n.json();return t?.clientPrincipal}}catch(n){return showError(n.message),null}}function showError(n){if(window.DotNet)try{DotNet.invokeMethodAsync("MM.WEB","ShowError",n)}catch{showToast(n)}else showToast(n)}function showToast(n){const t=document.getElementById("error-container");t&&(t.textContent=n,t.style.display="block",setTimeout(()=>{t.style.display="none"},5e3))}async function detectBrowserFeatures(){const[n,t,i]=await Promise.all([wasmFeatureDetect.simd(),wasmFeatureDetect.bulkMemory(),wasmFeatureDetect.bigInt()]);return n&&t&&i}function showBrowserWarning(){document.body.innerHTML=`
    ...
    `}(function(){const n=GetLocalStorage("theme")||"light";document.documentElement.setAttribute("data-bs-theme",n)})();