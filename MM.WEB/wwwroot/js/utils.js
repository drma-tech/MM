...
export const notification = {
    showError(message) {
        if (window.DotNet) {
            try {
                window.DotNet.invokeMethodAsync("MM.WEB", "ShowError", message);
            } catch {
...
export const environment = {
    ...
    async waitForAds(els, timeout = 15000) {
...
export const interop = {
    ...
    async updateLocation() {
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                if (!position?.coords) {
                    notification.showError("Unable to read location data.");
                    return;
                }

                const { latitude, longitude, accuracy } = position.coords;

                await interop.invokeDotNetWhenReady(
                    "MM.WEB",
                    "LocationChanged",
                    {
                        latitude,
