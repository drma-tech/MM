...
namespace MM.WEB.Core.Helper
{
    ...
    public class SwiperJs(IJSRuntime js) : JsModuleBase(js, "./js/swiper.js")
    {
        ...
        public Task InitGallery(string id) => InvokeVoid("swiper.initGallery", id);
    }
    ...
    public class PaymentsJs(IJSRuntime js) : JsModuleBase(js, "./js/payments.js")
    {
        ...
        public Task StripeOpenCheckout(string? priceId, int qtd) => InvokeVoid("stripe.openCheckout", priceId, qtd);
    }

    public class CropperJs(IJSRuntime js) : JsModuleBase(js, "./js/crop-helper.js")
    {
        public Task InitCropper(string imageId, int aspectRatio) => InvokeVoid("crop.initCropper", imageId, aspectRatio);

        public Task<string> GetCroppedImage(int width, int height) => Invoke<string>("crop.getCroppedImage", width, height);

        public Task<Modules.Profile.Components.SelectPicturePopup.ImageDimensions> GetImageSize(string imageUrl) => Invoke<Modules.Profile.Components.SelectPicturePopup.ImageDimensions>("crop.getImageSize", imageUrl);
    }
}