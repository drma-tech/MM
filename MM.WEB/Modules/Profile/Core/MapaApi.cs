...
public class MapApi(IHttpClientFactory http) : ApiCosmos<HereJson>(http, ApiType.Anonymous, null, [], ApiContext.Default.HereJson)
{
    ...
    {
        return await GetAsync(MapEndpoint.GetLocationHere(Latitude, Longitude), setNewVersion: false, state: null, cancellationToken);
    }

    public async Task<GoogleJson?> GetLocationGoogle(double Latitude, double Longitude, CancellationToken cancellationToken)
    {
        return await GetAsync<GoogleJson>(MapEndpoint.GetLocationGoogle(Latitude, Longitude), setNewVersion: false, state: null, cancellationToken);
    }
