...
public class VerificationFunction
{
      ...
      [HttpTrigger(AuthorizationLevel.Anonymous, Method.Post, Route = "public/didit/webhook")] HttpRequestData req, CancellationToken cancellationToken)
    {
        ...
        if (!DiditVerify.VerifySignatureV2(body, signature, timestamp, ApiStartup.Configurations.Didit?.SecretKey))
        {
        ...
        var payload = JsonSerializer.Deserialize<DiditResponse>(body, JsonSerializerOptions);
