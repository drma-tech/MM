...
public class StorageFunction(CosmosMainRepository repoGen, CosmosSafetyRepository repoSafety, CosmosProfileOffRepository repo, StorageHelper storageHelper, IHttpClientFactory factory)
{
        ...
        HttpRequestData req, CancellationToken cancellationToken)
    {
        ...
        if (responseAdult.ModerationLabels.Exists(x => x.Confidence >= 80f))
       ...
       [HttpTrigger(AuthorizationLevel.Function, Method.Put, Route = "storage/upload-photo-validation")] HttpRequestData req, CancellationToken cancellationToken)
    {
        ...
        await using var faceStream = await http.GetImageStreamFromUrlAsync(profile.GetPhoto(PhotoType.Face, live: false), cancellationToken);
        await using var bodyStream = await http.GetImageStreamFromUrlAsync(profile.GetPhoto(PhotoType.Body, live: false), cancellationToken);
