    ...
    public class CosmosProfileOffRepository
    {
        ...
        public async Task<List<T>> ListAll<T>(CancellationToken cancellationToken) where T : CosmosDocument
        {
            try
            {
                ...
                if (charges > 7)
                {
                    _logger.LogWarning("ListAll - ProfileOff, RequestCharge {Charges}", charges);
                }
        ...
        public async Task<T> PatchItem<T>(string? id, List<PatchOperation> operations, CancellationToken cancellationToken) where T : CosmosDocument, new()
        {
