...
public class StorageHelper(IConfiguration configuration)
{
    ...
    public async Task UploadPhoto(PhotoType type, Stream stream, string fileName, string userId,
        CancellationToken cancellationToken)
    {
        var container = new BlobContainerClient(Configuration.GetValue<string>("Azure:BlobConnectionString"),
            GetPhotoContainer(type));
        var client = container.GetBlobClient(fileName);

        var headers = new BlobHttpHeaders { ContentType = "image/jpeg" };

        await client.UploadAsync(stream, headers, new Dictionary<string, string> { { "id", userId } },
            cancellationToken: cancellationToken);
    }
